    ...
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg-base)' }}>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1rem' }}>
          <div style={{ width: 40, height: 40, border: '2px solid var(--accent-glow)', borderTopColor: 'var(--accent)', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />
          <style>{`@keyframes spin { to { transform: rotate(360deg); } }`}</style>
          <p style={{ color: 'var(--text-muted)', fontSize: '0.875rem', fontFamily: 'Inter, sans-serif' }}>Loading…</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div style={{
        ...
        <div style={{
          ...
          <h1 style={{ fontSize: '1.4rem', fontWeight: 700, color: 'var(--error)', margin: '0 0 0.75rem' }}>
          ...
          <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginBottom: '1.5rem' }}>
            Platform support: <strong style={{ color: 'var(--text-secondary)' }}>ashaan@platform.admin</strong>
          </p>
          <button
