...
function SalesTrendChart() {
  ...
  return (
    <div className="an-chart-card">
      ...
      <div className="an-chart-area" ref={chart.ref}>
        {loading ? (
          <div className="an-chart-loading"><span className="an-spinner" /> Loading chart data…</div>
        ) : error ? (
          <div className="an-chart-error">⚠ {error}</div>
        ) : data.length === 0 ? (
          <div className="an-chart-empty">No data available for this period.</div>
        ) : (
          <AreaChart width={chart.width} height={280} data={displayData} margin={{ top: 8, right: 16, left: 0, bottom: 0 }}>
              <defs>
    ...
    <div className="an-chart-card">
      ...
      <div className="an-chart-area" ref={chart.ref}>
        {loading ? (
          ...
          <BarChart width={chart.width} height={280} data={displayData} margin={{ top: 8, right: 16, left: 0, bottom: 8 }}>
...
function DuesPortfolioCard() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      setData(await fetchDuesPortfolio(range));
    } catch (e: any) {
    ...
    <div className="an-chart-card">
      <div className="an-chart-header">
        <div>
          ...
          <p className="an-chart-sub">Net dues activity recorded within the selected period</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <>
        {data.customer_dues > 0 || data.supplier_dues > 0 || data.employee_dues > 0 ? (
          <div className="an-chart-area" ref={chart.ref}>
            <BarChart width={chart.width} height={280} data={[
              { name: 'Customer', value: data.customer_dues, color: '#34d399' },
              { name: 'Supplier', value: data.supplier_dues, color: '#f87171' },
              { name: 'Employee', value: data.employee_dues, color: '#fbbf24' },
            ]} margin={{ top: 8, right: 16, left: 0, bottom: 8 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.05)" vertical={false} />
              <XAxis dataKey="name" tick={{ fill: '#64748b', fontSize: 11 }} axisLine={{ stroke: 'rgba(255,255,255,0.08)' }} tickLine={false} />
              <YAxis tickFormatter={value => formatRs(value)} tick={{ fill: '#64748b', fontSize: 11 }} axisLine={false} tickLine={false} width={72} />
              <Tooltip content={<CurrencyTooltip />} />
              <Bar dataKey="value" name="Dues" radius={[4, 4, 0, 0]}>
                <Cell fill="#34d399" />
                <Cell fill="#f87171" />
                <Cell fill="#fbbf24" />
              </Bar>
