...
// ─── Dues Portfolio (no date filter — these are running balances) ──────────
...
export async function fetchDuesPortfolio(): Promise<DuesPortfolio> {
  const [custRes, supRes, empRes] = await Promise.all([
  ...
  return {
    customer_dues: (custRes.data ?? []).reduce((s, r) => s + Math.max(0, Number(r.current_balance_due) || 0), 0),
    supplier_dues: (supRes.data ?? []).reduce((s, r) => s + Math.max(0, Number(r.current_balance_due) || 0), 0),
    employee_dues: (empRes.data ?? []).reduce((s, r) => s + Math.max(0, Number(r.remaining_balance) || 0), 0),
  };
}
 ...
 */
export async function fetchTenantStatusBreakdown(): Promise<TenantStatusBreakdown> {
  ...
  return { active, suspended, expired, total: active + suspended + expired };
}
